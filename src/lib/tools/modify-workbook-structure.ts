...
export const modifyWorkbookStructureTool = defineTool({
  ...
  parameters: Type.Object({
    ...
    newName: Type.Optional(
      Type.String({ description: "New name (rename) or name for copy (duplicate)" })
    ),
    rows: Type.Optional(
      Type.Number({ description: "Rows for new sheet. Default: 1000" })
    ),
    columns: Type.Optional(
      Type.Number({ description: "Columns for new sheet. Default: 26" })
    ),
